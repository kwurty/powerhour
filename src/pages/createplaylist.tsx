...
import { Video, Snippet, YoutubeResponseVideo } from '../types/youtubesearch.type'
import { useParams } from 'react-router-dom'
...
=======
  ...
  useEffect(() => {
    if (isEdit && id) {
      const getPlaylist = async () => {
        let response = await fetch(process.env.REACT_APP_BACKEND_API_BASE_API + 'playlists/' + id)
